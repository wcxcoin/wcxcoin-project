...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "WcxCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"104.236.56.195", "104.236.56.195"},
    {"104.131.174.87", "104.131.174.87"},
    {"138.197.46.182", "138.197.46.182"},
    {"104.236.27.15", "104.236.27.15"},
    {"104.236.27.15", "104.236.27.15"},
    {"104.131.52.61", "104.131.52.61"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("WcxCoin-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. WcxCoin is probably already running."), addrBind.ToString().c_str());
        else
